...
def set_group_tax(group: Group):
    ...
    for p in players:
        if p.treatment is not None:  # Ensure treatment is assigned
            ...
            disposable_income = p.current_income - (group.median_tax / 100) * p.current_income
    ...
    for p in players:
        p.final_income = p.after_tax_income + (group.total_tax_revenue / C.PLAYERS_PER_GROUP)
...
class Results(Page):
    def vars_for_template(self):
        return {
            'median_tax': self.group.median_tax,
            'current_income': self.player.current_income,
            'after_tax_income': self.player.after_tax_income,
            'final_income': self.player.final_income,
        }


page_sequence = [TaxChoice, ResultsWaitPage, Results]
